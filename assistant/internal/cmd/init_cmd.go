...
var (
	InitCmd = &gcmd.Command{
		...
		Func: func(ctx context.Context, parser *gcmd.Parser) (err error) {
			...
			fmt.Println("建议1：【第一次初始化】，请执行：xii up 或 xii up -d 来启动容器实例")
			fmt.Println("建议2：【新增】容器组件的，可以执行 xii up -容器组件 来单独启动容器组件，或者 xii up 直接启动所有容器组件,已存活的容器组件将被忽略")
			fmt.Println("建议3：【移除】容器组件的，可以执行 xii stop -容器组件 来停止容器组件，然后执行 xii rm -容器组件 来移除容器/镜像，然后手动删除配置文件，数据文件等（/data,/env/容器组件名）")
			fmt.Println("------------------")
			fmt.Println("\n\n")
